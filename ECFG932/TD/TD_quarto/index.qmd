...
26) Réaliser le graphique croisé des pénalités et des fréquences des attributs. On ajoutera :
...
27) Ajouter une colonne `Penalite_W` au data-frame `df_penalty`, calculant les pénalités pondérées. On fixera à 0 l'ensemble des pénalités qui ne sont pas pertinentes.
...
28) Réaliser alors un diagramme en barres des pénalités pondérées, et tracer un seuil déterminant si les défauts sont majeurs ou non.
...
29) Après avoir créer un individu fictif idéal, réaliser une **Analyse en Composantes Principales** *(ACP)* des évaluations JAR du jeu de données. Interpréter.
...
30) Réaliser cette même ACP, mais cette fois avec les données agrégées.
...
31) Créer un nouveau data-frame, ou le codage des attributs JAR est dédoublé :
...
32) Réaliser l'anlyse des correspondances multiples du jeu de données.
...
33) Après avoir agréger les données, ré-effectuer enfin une analyse des correspondances.
